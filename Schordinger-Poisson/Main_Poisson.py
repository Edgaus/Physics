...
while (np.min(error_phi)>1E-8) and (i<10): 
    ...
    inf_sheet_grid = inf_sheet_grid + ( delta_phi)
    i +=1

if i == 10:
    print('No convergió')

# --- (Opcional) Graficar el resultado final ---
plt.figure(figsize=(8,5))
plt.plot(x, Band_Edge_Potential_grid * 1000, 'k--', label='Pozo Original')
plt.plot(x, V_total * 1000, 'b-', label='Pozo Deformado (Schrödinger-Poisson)')
plt.axhline(values[0], color='r', linestyle=':', label=f'E0 = {values[0]:.1f} meV')
plt.legend()
plt.title("Perfil de Bandas Autoconsistente")
plt.xlabel("Posición (Å)")
