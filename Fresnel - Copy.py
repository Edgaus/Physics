...
class LayerStackApp(tk.Tk):
    def __init__(self):
        ...
        try:
            self.iconbitmap(r"Fresnel\Nueva carpeta\favicon.ico")
        except:
            pass # Ignore if the icon file isn't found
        ...
        self.opt_target = tk.StringVar(value="derivative")
        ...
        self.add_layer(material="AlN Film", thickness=100.0, fixed=False)
    ...
    def setup_styles(self):
        ...
        # Color Palette - deepest dark tone from plot background for matching areas
        BG = "#121212"       # Main window deep black
        FG = "white"         # Standard text color
        BLUE = "#0066CC"     # Accent color for titles/master calculate
        WIDGET_BG = "#333333" # Gray for inputs, as previously set. Optimization button will follow
        DEEP_DARK_BLACK = "#121212" # Matching plot's dark background exactly for unification
        ...
        # ==========================================
        # Unified Deep Dark Black Styling with White Borders
        # ==========================================
        
        # 1. Unified Backgrounds & White Borders
        # Apply deepest black to TFrame content areas to match plot, within standard borders
        # Widgets like spinbox, combox already have borders. Make fieldbackground matching deep dark black.
        # This will require custom widgets or further chameleon prompt specification if borders are desired around individual widget fields. Chameleon already shows borders in the prompt, so make fields matching DEEP_DARK_BLACK.
        
        # Toplevel windows might need this for consistency, but standard windows take BG by configure. 

        # TLabelframe content should also follow. Style TFrame already does it. Standard widgets *within* frame follow map below.

        # 2. Spinbox & Combobox Styling to match deep dark theme
        style.configure("TSpinbox", 
                        fieldbackground=DEEP_DARK_BLACK, # Matching deeply dark plot background for unity
                        background=DEEP_DARK_BLACK,      # Arrow area
                        foreground=FG,                   # White text
                        arrowcolor=FG,                   # White arrows
                        bordercolor="white",             # White border!
                        lightcolor="#222",
                        darkcolor="#222",
                        insertcolor=FG)                  # White blinking cursor
                        
        # How Spinboxes look when disabled or readonly
        style.map("TSpinbox",
                  fieldbackground=[("disabled", BG)],
                  foreground=[("disabled", "#555")],
                  background=[("active", DEEP_DARK_BLACK)]) # matching deepest dark! Standard clam active gray won't work well without description

        style.configure("TCombobox", 
                        fieldbackground=DEEP_DARK_BLACK, 
                        background=DEEP_DARK_BLACK, 
                        foreground=FG, 
                        arrowcolor=FG,
                        bordercolor="white",             # White border!
                        lightcolor="#222",
        ...
        # How Comboboxes look when disabled or readonly/active
        style.map("TCombobox",
                  fieldbackground=[("readonly", DEEP_DARK_BLACK), ("disabled", BG)],
                  foreground=[("disabled", "#555")],
                  background=[("active", DEEP_DARK_BLACK)]) # matching deepest dark! 

        # Combobox Dropdown List Styling
        self.option_add('*TCombobox*Listbox.background', DEEP_DARK_BLACK) # matching deep black dropdown list too for consistency
        self.option_add('*TCombobox*Listbox.foreground', FG)
        self.option_add('*TCombobox*Listbox.selectBackground', BLUE)
        self.option_add('*TCombobox*Listbox.selectForeground', FG)

        # Style standard entries similarly if needed. Standard fields used with ttk spin/combo are styled above.

        # Stylable buttons could follow, but standard Button can take bg/fg. Optimization green/red follow map/toggle. Save/load data standard style. Master blue style.
    ...
    def setup_layout(self):
        ...
        # ================= ROW 0 =================
        layer_box = ttk.LabelFrame(left, text="Estructura Multicapa", labelanchor='n', style="SubTitle.TLabelframe", padding=10)
        layer_box.grid(row=0, column=0, sticky="nsew", pady=15)
        
        self.layer_canvas = tk.Canvas(layer_box, bg="#121212", highlightthickness=0, height=330, width=440)
        ...
        # Labels not w sticky, widgets with correct configuration and dark style from setup_styles
        ttk.Label(ic_inner, text="λ Inicial (nm)").grid(row=0, column=0, padx=(30, 0), pady=(5, 0)) 
        ttk.Spinbox(ic_inner, textvariable=self.lambda_start, width=15, from_=200.0, to=2000.0, increment=1.0).grid(row=1, column=0, pady=5)

        ttk.Label(ic_inner, text="λ Final (nm)").grid(row=0, column=1, padx=(30, 0), pady=(5, 0)) 
        ...
        ttk.Label(ic_inner, text="Ángulo de Incidencia").grid(row=2, column=0, pady=(5, 0)) 
        ttk.Spinbox(ic_inner, from_=0, to=90, width=5, textvariable=self.theta_inc).grid(row=3, column=0, pady=5)

        ttk.Label(ic_inner, text="Polarización").grid(row=2, column=1, padx=(30, 0), pady=(5, 0))
        ...
        self.opt_frame = ttk.LabelFrame(left, text="Optimización ⚡", labelanchor='n', style="SubTitle.TLabelframe", padding=15)
        self.opt_frame.grid(row=2, column=0, sticky="ew", pady=10)
        self.opt_frame.columnconfigure(0, weight=1)

        self.opt_toggle_btn = tk.Button(self.opt_frame, text="🔴 Activar (Click)", bg="#333333", fg="white", font=("Segoe UI", 10, "bold"), relief="flat", cursor="hand2", command=self.toggle_optimization)
        ...
        ttk.Label(opt_inner, text="Función Objetivo").grid(row=0, column=0)
        ...
        ttk.Label(opt_inner, text="Algoritmo Optimización").grid(row=0, column=1, padx=(30, 0))
        ...
        ttk.Label(opt_inner, text="λ Inicial (nm)").grid(row=2, column=0, pady=(15, 0))
        ...
        ttk.Label(opt_inner, text="λ Final (nm)").grid(row=2, column=1, padx=(30, 0), pady=(15, 0))
        ...
        # 📂 Load Exp Data & 💾 Save Sim Data buttons
        ttk.Button(action_box, text="📂 Cargar Experimento", command=self.load_experimental_data).grid(row=0, column=0, sticky="ew", padx=2, pady=2)
        # Style standard ttk buttons dark might be hard, assume standard clam gray for now. Could style "TButton" but would affect Circular buttons if they were ttk. Need custom Circular anyway.

        ttk.Button(action_box, text="💾 Guardar Simulación", command=self.save_simulation_data).grid(row=0, column=1, sticky="ew", padx=2, pady=2)
        
        # ⚙️ Load State & ⚙️ Save State buttons
        ttk.Button(action_box, text="⚙️ Abrir Proyecto", command=self.load_state).grid(row=1, column=0, sticky="ew", padx=2, pady=2)
        ttk.Button(action_box, text="⚙️ Guardar Proyecto", command=self.save_state).grid(row=1, column=1, sticky="ew", padx=2, pady=2)
        
        # MASTER CALCULATE BUTTON (Absorbs Optimization)
        calc_btn = tk.Button(action_box, text="▶ CALCULAR / OPTIMIZAR", bg="#0066CC", fg="white", font=("Segoe UI", 11, "bold"), relief="flat", cursor="hand2", command=self.calculate)
    ...
    def add_layer(self, material="", thickness=0, fixed=False, index=None):
        layer = {
            ...
            "bound_thickness": tk.DoubleVar(value=100.0), # Default 100nm search range
            "fixed": fixed
        }
    ...
    def sync_opt_ui(self):
        if self.opt_active.get():
            self.opt_toggle_btn.config(text="🟢 Ativado: ON", bg="#2ecc71", fg="#121212")
            ...
            self.opt_toggle_btn.config(text="🔴 Desactivado: OFF", bg="#333333", fg="white")
            for w in [self.opt_cb_target, self.opt_cb_method, self.opt_spin_start, self.opt_spin_end]:
        ...
        try:
            ...
            for l in state["layers"]:
                layer = {
                    ...
                    "opt_thickness": tk.DoubleVar(value=l["thickness"]), # Optimized starts as thickness!
    ...
    def get_n_array(self, material_name, wave_m):
        if "Air" in material_name:
            ...
            try:
                # Load real (n) and complex (k) refractive index data from standard text files
                ...
                # Cubic interpolation across standard wavelength range
                ...
                # Return standard air refractive index (n=1, k=0) on failure
                return np.ones_like(wave_m) * 1.0 + 0j

    # ================== UPGRADED: CALCULATE & ZERO-BOUND FIX ==================
    def calculate(self):
        try:
            # Degrees to Radians!
            ...
            # Create a uniform wavelength array with exactly 1nm steps
            ...
            for layer in self.layers:
                ...
                if not layer["fixed"]:
                    ...
                    # --- NEW: THE ZERO-BOUND FIX ---
                    # If the user sets the bound to 0, make it microscopic 
                    # so SciPy doesn't crash, effectively freezing the layer.
                    if range_val == 0.0:
                        range_val = 1e-5 
                    
                    # Search window from max(0, Guess - Range) to Guess + Range
                    low = max(0, current_thick - range_val) 
                    high = current_thick + range_val
                    
                    thicknesses.append(current_thick)
                    # Pack bounds as tuples for SciPy optimizer
                    bounds.append((low, high)) 

            # Standard simulation: horizontal stack of n-arrays for standard and optimized cases
            ...
            if self.opt_active.get() == True:
                ...
                # Pack initial conditions including described data state. Theta arriving in radians.
                initial_conditions = {
                    'index_refr': n_stack,
                ...
                # Described range start/end from initial conditions spinboxes
                opt_range = [lam_i, lam_f]
                
                # Run the Optimizer
                # Separated exp_data x and y components as lists
                ...
                # Update the Tkinter UI directly with standard rounding to 2 decimal places
                opt_idx = 0
                for layer in self.layers:
                    if not layer["fixed"]:
                ...
                # Update plot with simulation (blue) and experimental (red dotted) lines
                self.update_plot(wave, best_plot_fit)
                messagebox.showinfo("Success", "Optimization complete!")
                
            else:
                # Standard Complex Fresnel Matrix simulation with described parameters
                ...
                # Update plot with standard solid blue simulation line and red dotted experimental line (if present)
    ...
    def on_close(self):
        # Force a clean matplotlib close
        plt.close('all')
        # Destroy the Tk window
        self.destroy()
        # Ensure the whole process terminates
        import os
        os._exit(0)
