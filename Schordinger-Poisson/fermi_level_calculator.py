...
def fermi_level_energy(Energies, Barrier_height, Nd, L_barrier, T, mas):
    ...
    n2D = (m_eff * kT) / (np.pi * hbar**2)  # Also works
    
    number_states = 1
    number_energy = len(Energies)
    Fermi_level_min = Energies[0] - 100 * kT
    Fermi_level_max = Barrier_height
    Fermi_level = 0
    
    donor_n_target = Nd * L_barrier  # Sheet density [m^-2]
    ...
    while number_states <= number_energy:
        ...
        if Fermi_level >= Fermi_level_max - kT:
            return Fermi_level, E_active, "spillover"
        ...
        # Converged
        return Fermi_level, E_active, "converged"
