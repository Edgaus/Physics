...
import ctypes # NUEVO: Para la barra de tareas
...
# --- ARREGLO DE LA BARRA DE TAREAS ---
try:
    myappid = 'proyecto.fresnel.edgaus.v4' 
    ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID(myappid)
except:
    pass

# --- FUNCIÓN PARA RUTAS EN PYINSTALLER ---
# --- FUNCIÓN PARA RUTAS EN PYINSTALLER (CORREGIDA) ---
def resource_path(relative_path):
    """ Busca la ruta correcta dependiendo si es un script o un .exe compilado """
    if getattr(sys, 'frozen', False):
        # Si es un .exe, busca exactamente en la carpeta donde está el .exe
        base_path = os.path.dirname(sys.executable)
    else:
        # Si es el script de Python, busca en la carpeta del script
...
class LayerStackApp(tk.Tk):
    def __init__(self):
        ...
        try:
            # Asegúrate de que tu archivo se llama "oco.ico" y está en la misma carpeta
            ruta_icono = resource_path("oco.ico")
            self.iconbitmap(default=ruta_icono)
        except:
            pass 
    ...
    def setup_layout(self):
        ...
        calc_btn.bind("<Enter>", lambda e: calc_btn.config(bg="#1A85FF"))
        calc_btn.bind("<Leave>", lambda e: calc_btn.config(bg="#0066CC")) 
    ...
    def update_plot(self, x_sim=None, y_sim=None):                                             
        ...
        self.ax.set_xlabel("Longitud de onda (nm)", fontsize=20, fontweight='bold', fontfamily='Segoe UI', labelpad=10) 
        self.ax.set_ylabel("Reflectancia (%)", fontsize=20, fontweight='bold', fontfamily='Segoe UI', labelpad=15) 
    ...
    def get_n_array(self, material_name, wave_m):
        if "Air" in material_name:
            ...
            try:
                ...
                data_k = np.loadtxt(ruta_k, delimiter="\t", skiprows=1)
                ...
                # Si falla, ahora te avisará en lugar de hacer una gráfica en cero
                print(f"Error cargando: {e}")
                messagebox.showerror("Error de Material", f"Falta el archivo:\n{ruta_n}")
                return np.ones_like(wave_m) * 1.0 + 0j
